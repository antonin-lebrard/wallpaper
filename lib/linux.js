...
function convertToGnomePictureOptions(scale) {
	if (scale === "center")
		return "centered";
	if (scale === "fit")
		return "scaled";
...
exports.set = function set(imagePath, opts) {
	if (typeof imagePath !== 'string') {
		return Promise.reject(new TypeError('Expected a string'));
	}

	if (!availableApps) {
		return setAvailableApps().then(() => set(imagePath, opts));
	}
	...
	if (app.cmd === 'gsettings') {
		let scale = convertToGnomePictureOptions(opts.scale);
		let isCinnamonEnv = false;
		if (!scale)
			scale = "centered";
		return isCinnamon()
			.then(cinnamon => {
			...
			.then(() => {
				const drawBackgroundParams = [params[0], params[1], 'draw-background'];
				const pictureOptionsParams = [params[0], params[1], 'picture-options'];
				if (!isCinnamonEnv) {
					return cp.execFile(app.cmd, drawBackgroundParams.concat('false'))
						.then(() => cp.execFile(app.cmd, params))
						.then(() => cp.execFile(app.cmd, drawBackgroundParams.concat('true')))
						.then(() => cp.execFile(app.cmd, pictureOptionsParams.concat(scale)));
				} else { // isCinnamon, doesn't seems to have drawBackground param for gsettings
